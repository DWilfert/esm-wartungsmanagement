...
def zeige_5jahresplan():
    ...
    # --- DYNAMISCHE FARBEN FÜR PLOTLY & HTML (Inkl. Fullscreen-Fix) ---
    ...
    if conn is not None:
        try:
            sql_uebersicht = "SELECT id, anlagenid, bezeichnung, firma, standort, zyklusmonate, letztewartung, naechstewartung FROM `wartungsvertraege`"
            df_ueber = pd.read_sql(sql_uebersicht, conn)
                                ...
                                if not pd.isnull(basis_datum):
                                    with col_m2:
                                        ...
                                        if st.button(btn_back_lbl, use_container_width=True, key=f"btn_plus_{v_schieb_id}"):
                                            neues_datum = (basis_datum + timedelta(days=28)).strftime('%Y-%m-%d')
                                            conn_db = hole_datenbank_verbindung()
                                            if conn_db:
                                                cursor = conn_db.cursor()
                                                cursor.execute("UPDATE `wartungsvertraege` SET naechstewartung = %s WHERE id = %s", (neues_datum, v_schieb_id))
                                                conn_db.commit()
                                                conn_db.close()
            ...
            err_lbl = "Fehler:" if st.session_state.language == "de" else "Error:"
            st.error(f"{err_lbl} {str(e)}")
        finally: 
            conn.close()